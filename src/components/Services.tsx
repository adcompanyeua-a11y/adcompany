      ...
      <div className="container mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {t.services.items.map((item, i) => {
            const Icon = icons[i];
            const isGoogle = Icon === GoogleIcon;
            return (
              <div key={item.title} className="group relative p-8 rounded-2xl bg-card-grad border border-border hover:border-brand-yellow/50 transition-smooth shadow-card-soft hover:-translate-y-1">
                <div className={`inline-flex h-14 w-14 items-center justify-center rounded-xl mb-5 shadow-yellow-soft group-hover:scale-110 transition-smooth ${isGoogle ? "bg-white" : "bg-yellow-grad text-brand-navy"}`}>
                  {isGoogle ? <Icon className="h-7 w-7" /> : <Icon className="h-7 w-7" strokeWidth={2.5} />}
                </div>
